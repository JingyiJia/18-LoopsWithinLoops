...
def draw_L(window, circle, r, c):
    ...
    for k in range(r):
        for j in range(3):
            n_circle = rg.Circle(rg.Point(x,y),radius)
            n_circle.fill_color = circle.fill_color
    ...
    for k in range(3):
        for j in range(c+3):
            n_circle = rg.Circle(rg.Point(x,y),radius)
            n_circle.fill_color = color.fill_color
            n_circle.attach_to(window)
            windwo.render(0.1)
            x = x + 2 * radius
        y = y + 2 * radius
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for k in range(n):
        for j in range(k+1):
            n_rectange = rg.Rectangle(rg.Point(a,b),rg.Point(c,d))
            n_rectange.attach_to(window)
            window.render(0.1)
